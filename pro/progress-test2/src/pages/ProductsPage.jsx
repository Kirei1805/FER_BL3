  ...
  useEffect(() => {
    (async () => {
      ...
      const normalized = (data || []).map(p => ({
        id: p.id,
        name: p.title || p.name,
        image: p.image?.startsWith('/') ? p.image : `/${p.image}` || `https://picsum.photos/seed/${p.id}/600/400`,
        price: p.price,
  ...
  return (
    <>
      ...
      <Container className="py-4">
        <h1 className="mb-4">Danh sách điện thoại</h1>
      ...
      {filteredProducts.length === 0 && (
        <div className="mt-4">
          <Card className="text-center">
            <Card.Body>
              <div className="py-5">
                <h5 className="text-muted mb-3">🔍 Không tìm thấy điện thoại nào</h5>
